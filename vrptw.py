...
from src.localSearch import localSearch, printSolution, initialSolution
...
if __name__ == '__main__':
	...
	sol = localSearch(GRAPH, CAPACITY, DEPOT, NODES, NVEHICLES)
	print(sol)
	"""
	##lancia i processi
	nprocesses = 8
	#print(CAPACITY, DEPOT, NODES, NVEHICLES)
	pool = multiprocessing.Pool(processes = nprocesses)

	solutions = pool.starmap(localSearch, [(GRAPH, CAPACITY, DEPOT, NODES, NVEHICLES)]*nprocesses)

	solutions = [elem for elem in solutions if elem != (None, None, None) ]

	if len(solutions) == 0:
		print("Could not find feaseble solution")
		exit()

	solutions.sort(key=lambda elem: elem[1])
	toc = time.time()
	...
	#istogramma delle soluzioni
	"""returns = pool.starmap(createSolutions, [(GRAPH, CAPACITY, DEPOT, NODES, NVEHICLES, 200)]*8)
	values = [elem2 for elem in returns for elem2 in elem]
	print(values)
	plt.hist(values, max(values) - min(values))
	plt.show()"""
